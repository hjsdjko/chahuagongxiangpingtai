<template>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in chahuaTypesList" :key="index"
    ...
    <view
        ...
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.chahuaPhoto?product.chahuaPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.chahuaNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.chahuaNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('chahua','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('chahua','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('chahua','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`chahua`, params);
			...
			async search() {
                ...
                let res = await this.$api.list(`chahua`, searchForm);
				// 如果是第一页数据置空
